...
def build_mcp_app() -> Optional[Any]:
    ...
    # Some non-interactive clients cancel direct MCP tool calls under a
    # never-approve policy while still allowing read-only MCP resources.  This
    # fixed resource invokes the same registered verification implementation;
    # it is a transport bridge, not a substitute verifier.
    if hasattr(app, "resource"):
        @app.resource("reasoning-agent://verification/tutorial_square")
        def _resource_verify_tutorial_square() -> str:
            problem_file = REPO_ROOT / "data" / "tutorial_square.md"
            blueprint = REPO_ROOT / "results" / "tutorial_square" / "blueprint.md"
            statement = problem_file.read_text(encoding="utf-8").strip()
            if statement.startswith("# Problem"):
                statement = statement[len("# Problem") :].strip()
            result = verify_proof_service(
                statement=statement,
                proof=blueprint.read_text(encoding="utf-8"),
            )
